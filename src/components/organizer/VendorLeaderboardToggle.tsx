...
import { toggleVendorLeaderboard } from '@/actions/vendor';

export function VendorLeaderboardToggle({ festivalId, initialEnabled, entitled = true }: { festivalId: string, initialEnabled: boolean, entitled?: boolean }) {
  ...
  const handleToggle = async () => {
    ...
    try {
      ...
      await toggleVendorLeaderboard(festivalId, newState);
      setEnabled(newState);
      router.refresh();
    } catch (error) {
      console.error('Failed to toggle vendor leaderboard:', error);
    } finally {
      setIsLoading(false);
    }
  };
  ...
  return (
    <div className="p-6 border border-border bg-card rounded-xl flex items-center justify-between mt-4">
      <div className="flex items-start gap-4">
        ...
        <div>
          <h3 className="font-bold">Vendor Leaderboard</h3>
          <p className="text-sm text-muted-foreground mt-1 max-w-lg">
            Enable a public leaderboard for vendors to see how they rank against others. Vendor exact numbers can be optionally hidden.
          </p>
          {!entitled && (
